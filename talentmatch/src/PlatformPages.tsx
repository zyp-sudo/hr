...
import { Activity, AlertTriangle, ArrowLeft, ArrowUpRight, BriefcaseBusiness, CalendarDays, CheckCircle2, CircleDot, Clock3, GitCompareArrows, Heart, Info, Lock, Mail, MapPin, Plus, RefreshCw, Route, Search, Sparkles, Trash2, TrendingUp, UserRound, X } from "lucide-react";

export type PageKey="overview"|"unified"|"graph"|"compare"|"evolution"|"jobs"|"favorites"|"about";
...
type InterviewRow={id:string;candidateId:string;candidateName:string;jobTitle:string;scheduledAt:string;interviewer:string;status:string};
...
const meta:Record<PageKey,{eyebrow:string;title:string;desc:string}>={
  ...
  graph:{eyebrow:"TALENT CAPABILITY GRAPH",title:"人才图谱",desc:"拖动和点击能力节点，查看候选人的能力、缺口与岗位适配关系。"},
  ...
  favorites:{eyebrow:"FAVORITE TALENTS",title:"特别关注",desc:"集中跟进重点候选人的档案、匹配结果与人才图谱。"},
        ...
        <section className="home-section" style={{marginBottom:20}}>
          <div className="home-insights">
            <div className="home-quote-card">
              ...
              <p className="home-quote-card__text">{'登录后即可访问所有功能：岗位管理、人岗匹配、人才图谱、评估结果、招聘趋势等。'}</p>
            ...
            <div className="home-insight-card">
              ...
              <ul className="home-insight-card__list">
                ...
                <li className="home-insight-card__item"><div className="home-insight-card__dot"/><div className="home-insight-card__item-content"><h4>{'人才图谱'}</h4><p>{'可视化候选人的能力、缺口与岗位适配关系。'}</p></div></li>
  ...
  return <div className="platform-main" style={{padding:0,margin:0,maxWidth:"100%"}}>
    ...
    <div style={{maxWidth:1280,margin:"0 auto",padding:"0 32px 32px"}}>
      ...
      <div className="home-stats">
        ...
        <button className="home-stat" onClick={()=>onNavigate?.("overview")}>
...
function TalentGraphPage({onCompare,preferredCandidateName}:{onCompare:()=>void;preferredCandidateName?:string}){
...
function JobsPage({workspace,onNavigate}:{workspace:boolean;onNavigate?:(page:PageKey|"matching")=>void}){
  ...
  return <><div className="page-actions"><span><Clock3/>更新时间：{updated}</span>{!workspace&&<button className="ghost-action spring-hover" onClick={()=>load(1,false,query)} disabled={loading}><RefreshCw className={loading?"spin":""}/>{loading?"刷新中":"刷新列表"}</button>}{!workspace&&<button className="ghost-action ai-refresh" onClick={runCrawler} disabled={crawler?.status==="running"}><Sparkles/>{crawler?.status==="running"?"采集并同步中":"快速采集"}</button>}<button className="primary small-primary" onClick={()=>setAdding(true)}><Plus/>新增岗位</button></div>
    ...
    <section className="stats-grid"><Stat label="在招岗位" value={total+localJobs.length} onClick={()=>document.querySelector(".job-card-grid")?.scrollIntoView({behavior:"smooth"})} help="数据库中的真实岗位总量"/><Stat label="候选人" value={candidates.length} tone="mint" onClick={()=>setCandidatePanel(true)} help="点击查看和添加候选人"/><Stat label="待安排面试" value={interviews.filter(x=>x.status==="待进行").length} tone="cyan" onClick={()=>setInterviewPanel(true)} help="点击管理面试安排"/><Stat label="平均匹配度" value={average===null?"--":`${average}%`} onClick={()=>onNavigate?.("unified")} help={average===null?"暂无真实评估记录":"真实评估总分 ÷ 评估人数"}/></section>
...
function InterviewDialog({candidates,interviews,jobs,close,onChange}:{candidates:CandidateRow[];interviews:InterviewRow[];jobs:JobRow[];close:()=>void;onChange:()=>void}){const [form,setForm]=useState({candidateId:candidates[0]?.id||"",jobTitle:jobs[0]?.name||"",scheduledAt:"",interviewer:""});const add=async()=>{const c=candidates.find(x=>x.id===form.candidateId);await json("/api/hr/interviews",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,candidateName:c?.name||"匿名候选人"})});onChange()};return <div className="modal-backdrop"><div className="dialog large-dialog"><div className="dialog-head"><div><span>INTERVIEW SCHEDULE</span><h3>面试安排</h3></div><button onClick={close}><X/></button></div><div className="interview-form"><label><span>候选人</span><select value={form.candidateId} onChange={e=>setForm({...form,candidateId:e.target.value})}>{candidates.map(c=><option value={c.id} key={c.id}>{c.name||"匿名候选人"}</option>)}</select></label><label><span>目标岗位</span><select value={form.jobTitle} onChange={e=>setForm({...form,jobTitle:e.target.value})}>{jobs.slice(0,30).map(j=><option value={j.name} key={j.id}>{j.name}</option>)}</select></label><label><span>面试时间</span><input type="datetime-local" value={form.scheduledAt} onChange={e=>setForm({...form,scheduledAt:e.target.value})}/></label><label><span>面试官</span><input value={form.interviewer} onChange={e=>setForm({...form,interviewer:e.target.value})} placeholder="姓名或团队"/></label><button className="primary" disabled={!form.candidateId||!form.scheduledAt} onClick={add}>安排面试</button></div><div className="interview-list">{interviews.map(x=><div key={x.id}><CalendarDays/><span><b>{x.candidateName} · {x.jobTitle}</b><small>{timeText(new Date(x.scheduledAt))} · {x.interviewer}</small></span><em>{x.status}</em></div>)}</div></div></div>}
