...
Write-Host "==> Starting MySQL, Neo4j, Elasticsearch and Milvus..." -ForegroundColor Cyan
...
$MilvusContainer = if ($env:MILVUS_CONTAINER_NAME) { $env:MILVUS_CONTAINER_NAME } else { "xh-milvus" }
$ready = $false
for ($i = 0; $i -lt 60; $i++) {
  $states = docker inspect --format '{{.State.Health.Status}}' $MySQLContainer $Neo4jContainer $ESContainer $MilvusContainer 2>$null
  Assert-NativeSuccess "docker inspect database health"
  if ($states.Count -eq 4 -and ($states | Where-Object { $_ -ne "healthy" }).Count -eq 0) {
    $ready = $true
    break
  }
